...
int	  SIGEL_Simulation::SIG_DynaMechsSimulationQueries::getNumberOfTouchdowns( void ) const
{	SIG_DynaMechsLink	*sigDMLink;
	dmLink						*internalLink;
	dmContactModel		*contactModel;
	dmRigidBody				*rBody;
	...
	// iterate through all links and get count of links touching the floor
	tdLinks = 0;

  for (i=0; i<simulationData.dynaMechsLinks.size(); i++)
	{
		sigDMLink = simulationData.dynaMechsLinks[i];
		rBody = sigDMLink->dynaMechsLink;
		...
		//	check all points/vertices of current link for ground contact
		j = 0;
		hasContact = false;

		while (j < contactModel->getNumContactPoints())
		{
			if (contactModel->getContactState(j))
			{ hasContact = true;
			  break;
			}
