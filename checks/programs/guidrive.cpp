/*
  ...
  metadrive, runlock, rngseed, pvmcrash, openfocus.
  The list above is not maintained in step with the code. The count that
  cannot go stale is
    command grep -o 'scenario == "[a-z]*"' checks/programs/guidrive.cpp \
        | sed 's/.*"\(.*\)"/\1/' | sort -u | wc -l
  which reads 30 today. A plain -c over the same pattern gives 34, which is a
  count of LINES rather than of matches (`grep -o | wc -l' gives 38): four
...
// and watched the whole gate pass. The forms section covers only .qrc-backed
