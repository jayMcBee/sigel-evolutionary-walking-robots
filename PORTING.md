...
| **D36** *(signed off 2026-09-15)* | Dialogs out of sight | **No SIGEL dialog may end up out of sight and block the window.** This holds whatever the save rules become. **The interface half is done — D39**, all 32 sites in `SIGEL_MasterGUI`, after D35 gave the eight save and export file dialogs a parent. The rest was done on 2026-09-22, item 25, apart from the six prompts in `MT_StatisticsWidget` that item 33 is to delete |
...
| **D38** *(signed off 2026-09-16; revised 2026-09-24, item 24)* | The generation progress bar | **`generationProgBar` counts the generation's finished tournaments, out of the tournaments `createTours` planned.** The widget sat in `SIG_ExperimentViewBase.ui` since before 1.3 and nothing ever wrote to it. A `QTimer` in `SIG_GUIGPExperiment` reads `SIG_GPManager::tournamentProgress()` and sets the bar's range and value; it starts in `slotStartEvolution` and stops in `slotEvolutionStopped`. `tournamentProgress` is one const method: planned is `tours.size()`, and a tournament is unfinished while its `depNumber` is above 0 or it is in `taskCanDoList`. It adds no state and changes nothing in the run. The bar only rises within a generation and is full at its end. **Before the first tournaments exist both counts are 0 and Qt shows a busy indicator**, which covers the first generation's pool evaluation. In later generations the bar stays full through `evalNewIndis`. **It runs ahead when reproduction is above 0**, because a reproduction tournament needs no evaluation; twoBases is the kept experiment that has it. **MetaGP is not covered**, by decision. The first version counted individuals with a fitness and stepped back every time a tournament wrote offspring. Tried in use and accepted 2026-09-24. **Not checked:** no check starts a run |
...
and `checks/replicate.sh` were deleted; **966** the same day, when six checks
that guarded only the Qt 6 conversion were dropped: `widgets`, `parsers`,
...
forms. **It also RUNS SIGEL**: **twenty
`guidrive` invocations over thirteen distinct scenarios**, nineteen where no
comma-decimal locale is installed — the eleven of
`gui behaviour`, plus **two locale re-runs of `pages`**, `xtest`, `pagesave` twice, and **`pagesave` four more times in
...
- **Changed:** `SIG_GPFitnessFunction` has a pure virtual `serializedId()`.
  ...
  "ZorcWalkingFitnessFunction", the ID that experiment files store. The six
  getters are inline in the class declarations. This is the first step of
  item 85, with 84 and 83.
- **Baselines:** unchanged. `fitness-check.sh` is identical to its baseline.
...
- **Changed, by decision:** Save writes without a dialog; a new experiment
  ...
  entry in "Done". Checked on the desktop with a copy of `runner.exp`.
- **`guidrive`:** four scenarios that need the dialog click Save As; `rngseed`
  saves a new experiment and still clicks Save.
- **Baselines:** `guibehaviour-baseline.txt` and `xtest-baseline.txt` were
  regenerated by running the same scenarios; their comment lines are
  unchanged. A script matched every changed line to a cause: 14 new menu
  entry lines, the action count 58 to 59, the locked-action count 29 to 30,
  and the dialog title "Save Experiment As". Two runs gave identical output.
- **Review:** no defects. It noted the Rename behaviour in the Done entry.
- **Gates:** `check.sh` 936 pass, 0 fail; warnings 487. The other four gates
  are green.
- **Next:** item 70, a menu command that clones an experiment with a fresh
  population.

**2026-09-25 — DONE: A LENGTH COLUMN ON THE INDIVIDUALS PAGE.**

- **Added, by decision:** a "Length" column between Fitness and Age, the
  number of program lines. It sorts by number and fits its contents, at
  least fourteen digits wide. `SIG_IndividualListItem::setTo` fills it, so it
  follows every generation; the second constructor now calls `setTo` instead
  of repeating it, and `key()` serves Length and Age with one numeric case.
  Checked on the desktop with a copy of `runner.exp`.
- **`guidrive`:** seven prints of an individual's age read column 2; they
  read column 3 now. The first gate run failed on them: the five random
  individuals the Add dialog creates have a random length, where their age
  is always 0.
- **Baselines:** `guibehaviour-baseline.txt` and `xtest-baseline.txt` were
  regenerated by running the same `guidrive` scenarios; their comment lines
  are unchanged. A script compared them with the committed ones: all 80 and
  8 changed lines are the table dump, and each differs only by the new column
  or by the checksum over all cells. Two runs gave identical output.
- **Review:** no defects.
- **Gates:** `check.sh` 936 pass, 0 fail; warnings 487. The other four gates
  are green.
- **Next:** the remaining `const_cast`s, 25 in six modules.

**2026-09-25 — DONE: FIVE READ-ONLY `SIG_Program` METHODS ARE CONST.**
...
- **Removed:** `SIG_SimulationVisualisationWidget::mousePressEvent` and
  ...
  which Qt now reaches directly. The base class comments no longer ask
  subclasses to override them.
- **Gates:** `check.sh` 936 pass, 0 fail; warnings 491. The other four gates
  are green.

**2026-09-25 — DONE: THREE UNCALLED OR FORWARDING METHODS REMOVED.**
...
- **Dropped, by decision:** the `truncated pi (V5)` section. The truncated pi
  ...
  needs `sigel_eval`; `fitness-check.sh` does.
- **Gates:** `check.sh` 963 pass, 0 fail; warnings 497.

**2026-09-24, night — DONE: SEVEN CHECKS DROPPED FROM `check.sh`.**

- **Dropped, by decision:** `widgets`, `parsers`, `freed-pointer null`,
  `no clipped controls`, `form minimums` and `slave gui`. Each guarded one
  defect of the Qt 6 conversion, and each is fixed. They can only fail when
  that code, a form or Qt itself changes. `guidrive` lost the three scenarios
  only they ran. Also `gui vs 1.3` and `guidump-baseline.txt`: `gui behaviour`
  already prints the menu text and every action's enabled state.
- **What is lost:** an automatic warning when a Qt 6 update changes spin box,
  stream or layout behaviour, and the runtime check of `SIGEL_SlaveGUI`'s
  string-based connects. The checks are in git history for that case.
- **New item 88:** any load error kills the interface.
- **Gates:** `check.sh` 964 pass, 0 fail; warnings 497.

**2026-09-24, evening — DONE: ITEM 71, THE REGISTER WIDTH.**
...
- **Item 25 is done.** Every dialog outside item 33's six prompts has a
  ...
  row says so. Details in the Done entry for item 25.
- **D43 is on `main`.** It was parked on branch `d43-unfinished` on
...
- **The `evolution` scenario must start in `sigelApp/`**, with the launcher's
  ...
  `supportingLibs/pvm3/bin/LINUX64/sigel_slave`, as `sigelLauncher` sets them.
  Started from the repo root, no slave starts and the run waits for ever.
- **`check.sh` passes 973**, since item 25's new header. Two steps on 2026-09-22, each proven by a clean
...
not seconds**: the run blocks the main thread, the only pump was the
`processEvents` in `haveABreak()` (now `processInterfaceEvents`), once per outer pass, with `usleep(300000)` per tournament touch
...
| **The duplicate MetaGP About is removed**, with its trailing separator. 1.3 has 43 menu items, the port 42 | a decision, 2026-09-02 — the port's first intentional behavioural difference. Not a defect, not a regression | `guidump-baseline.txt` until 2026-09-24; since then `gui behaviour`, whose action list would gain a row. The separator alone is not seen |
...
by accident: 1.3's TRUNCATED PI.** The pitch/roll sensor path converts radians to degrees
with `3.14159265`, so its factor is `180/3.14159265` and not `180/pi` — off by
1.14e-09. *Since 2026-09-23 the joint-sensor branch uses no factor (the joint-sensor
fix of the 1.0 → 1.3 regression). The constant is left at three sites: the pitch/roll
branch of `sense` and `moveDrive`'s servo mode. No shipped file uses either.* **The correct constant is absent from both 1.3
...
the source and in the built binary, until 2026-09-24. Evidence in
...
| **`runtime connect`** — NEW 2026-09-03 | Qt's own `No such signal` / `No such slot` warning, from a driver run — since 2026-09-22 the only check of string-based connects. `guidrive`'s stderr had been going to `/dev/null`; it is captured now and any such line fails the check. It sees only what the scenarios execute; the static regex over all 14 modules was dropped on 2026-09-22. What THIS row does not reach: the two locale re-runs append to the log *after* it has been grepped, and `SIGEL_SlaveGUI` — 44 `SIGNAL(` and 44 `SLOT(` with no runtime coverage since `slave gui` was dropped on 2026-09-24. *There are three such checks in `check.sh` now (`gui behaviour`, `pagesave`, `v2 round trip vs 1.3`), one per stderr stream, and they are deliberate duplicates: each stream has a different producer* |
...
- [x] **23. The window stops answering during a run** — done 2026-09-18, D43,
...
- [x] **25. Dialogs with no parent can end up out of sight** — done
  2026-09-22. Found when `SIG_ExperimentListView::slotSaveExperiment`'s
  overwrite prompt sent `set_modal` and no `set_parent` on Wayland, and blocked
  the window. D39 had given 32 dialogs in `SIGEL_MasterGUI` a parent. The rest
  now have one:
  - `SIG_MainWindow::slotChangeFont`'s font dialog, which D39 missed: the main
    window.
  - The MetaGP window, which is application-modal: `SIG_MainWindow` passes
    itself to `MT_Controller::configureSystem` as the owner. The main window is
    never deleted, so `MT_Controller` stays the only code that deletes the
    MetaGP window.
  - `MT_Controller::slotSaveSetup`'s prompt: the MetaGP window.
  - Four more in `MT_Controller`, the two `QProgressDialog` in
    `SIG_GPPopulation`, the `QInputDialog` in `SIG_GPRemoteZORCFitnessFunction`
    and the three texture warnings in `SIG_EnvironmentRenderer`. These classes
    hold no window, so they call `SIGEL_Tools::dialogParent()`: the open modal
    dialog, else the active window, else the visible main window —
    `SIG_MainWindow`, or the slave's simulation window. `activeWindow()` alone
    is null for a moment after another dialog closes, and that is when
    `MT_Controller::readFromFile`'s prompt runs after the Open dialog,
...
- [x] **79. File > Save Experiment always asked for a file name** — done
  ...
  autosave also uses. An experiment that has no file yet, path `"new"`, goes
  to the new `slotSaveExperimentAs`, which is the old Save: it asks for a
  name, writes, and sets the name and the path. "Save Experiment &As..." is
  in the File menu and in the experiment list's right-click menu, which opens
  on the empty area below the experiments. It has no toolbar button and no
  shortcut, because Ctrl+Shift+S is Import > Simulation-Parameters. It is
  greyed with no experiment and during a run. Not changed: a file that cannot
  be opened for writing still gives no message. After Rename, Save writes to
  the old file, as autosave always did.
...
- [x] **24. Show progress during a run** — done 2026-09-24, D38 revised.
  The bar counts finished tournaments out of planned ones, read through
  `SIG_GPManager::tournamentProgress()`. Rejected on the way: counting
  evaluations (the interface would have to know each operator's number of
  offspring), counters inside the loop, and every estimate from the first
  round. Counting individuals of age 0 does not work either: an offspring can
  be overwritten in the same generation, and the total is not in the
  population. Tried in use and accepted; one independent review, no defects.
  **Kept from the item, about the generation counter (D37):** the matching
  `updateIndividualView` call in the `SIG_GUIGPManager` constructor stays
  commented out, as in 1.3, because `slotStartEvolution` already refreshes the
  counter through `putAllIntoExperiment`. No check covers the counter during a
...
during a run re-entered the slot through `haveABreak`'s (now `processInterfaceEvents`) `processEvents` and ran
...
- **`slotStartEvolution` BLOCKS, so a sampling loop written after the click sees
  ...
  inline, and the GUI survives only because
  `SIG_GUIGPManager::processInterfaceEvents()` calls `qApp->processEvents()`. Anything that must observe a RUNNING evolution
...
correctly recorded *and deliberately kept* (until 2026-09-23). Each is right on its own; the
...
expression.** The pitch/roll branch of `SIG_DynaMechsSimulationQueries::sense`
writes `360.0 / (2.0*3.14159265)` twice, which constant-folds to
`0x404ca5dc1af05a77` — **the identical bit pattern** to the constant baked into
the 1.3 binary. Verified, not assumed. *Since 2026-09-23 the joint-sensor branch
has no factor: the joint-sensor fix of the 1.0 → 1.3 regression.*

**A change to this constant is a change of behaviour.** Replacing `3.14159265`
with `M_PI` or `4*atan(1)` shifts every pitch/roll reading and servo target by
~1.1e-9 relative. That is invisible in a 6-significant-digit file and irrelevant
to a static comparison — **and it feeds the simulation**, where it is exactly the
kind of perturbation that amplifies. No shipped file uses pitch/roll sensors or
servo drives.
...
| `scaledState = (q - minPos)` then `*= 57.2957…` | **`+inf`** — the multiply comes first and `1.8e308 × 57.3` overflows |
| `scaledState /= posRange` | **`NaN`** — `inf / inf` |
| `static_cast<int>( scaledState * … )` | **undefined behaviour** |

**And the 1.3 regression in `sense`'s joint-sensor branch is what makes it
...
So the regression also converts a quiet failure into an undefined one, on an
input no shipped file produces. *Since 2026-09-23 the port divides first (the
joint-sensor fix of the 1.0 → 1.3 regression) and gives 0 here. The NaN and the undefined
behaviour in the table above are 1.3's only.*
...
**This is 1.3's defect, and the port still has it:** the sentinel still does
not survive a save and load. Since the joint-sensor fix the result is a
constant 0, not NaN. 1.3
