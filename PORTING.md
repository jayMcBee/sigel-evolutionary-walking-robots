...
| **4** | **Six dropped size constraints from C2 were never restored — and this class is NOT cosmetic.** §7 says "C6/C7 own it"; both closed without it. Measured: `MT_IndividualWidgetBase.ui` has three `130` values in `v1.3-pristine` and zero today, and `Layout32/33/28/60/22` appear in no converted form. **A sibling case found on 2026-09-03 HID CONTROLS FROM THE USER**: the `Navigation` group in `SIG_SimulationWidgetBase.ui` rendered **90x37** against 1.3's **220x331**, clipping away all seven 50x50 navigation buttons and the X/Y/Z position readouts. Cause: `sizePolicy` Fixed with children placed by ABSOLUTE geometry and no layout — Qt 2 sized such a group from its children, Qt 6 from a sizeHint that ignores them, so it collapses. *That one was not a dropped value: the pristine form carries no size either, so it is a Qt behaviour difference needing a NEW minimum, not a restoration.* Fixed by giving the group 1.3's measured 220x331. | open for the remaining six. **Reclassified from cosmetic to user-facing** until each is measured: the test is whether a container's children need a bigger box than it declares, which is arithmetic and can be done over every converted form without running anything |
